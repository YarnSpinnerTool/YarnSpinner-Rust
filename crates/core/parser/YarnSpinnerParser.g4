...
options {
	tokenVocab = YarnSpinnerLexer;
}

dialogue: (file_hashtag*) node+;

// File-global hashtags, which precede all nodes
file_hashtag: HASHTAG text = HASHTAG_TEXT;

node: header+ BODY_START body BODY_END;
...
line_condition: COMMAND_START COMMAND_IF expression COMMAND_END;
...
function_call: FUNC_ID '(' expression? (COMMA expression)* ')';
...
else_clause: COMMAND_START COMMAND_ELSE COMMAND_END statement*;

set_statement:
	COMMAND_START COMMAND_SET variable op = (
		OPERATOR_ASSIGNMENT
		| '*='
		| '/='
		| '%='
		| '+='
		| '-='
	) expression COMMAND_END;

call_statement:
	COMMAND_START COMMAND_CALL function_call COMMAND_END;

command_statement:
	COMMAND_START command_formatted_text COMMAND_TEXT_END (
		hashtag*
	);

command_formatted_text: (
		COMMAND_TEXT
		| COMMAND_EXPRESSION_START expression EXPRESSION_END
	)*;

shortcut_option_statement:
	shortcut_option* (
		shortcut_option BLANK_LINE_FOLLOWING_OPTION?
	);
...
jump_statement:
	...
	| COMMAND_START COMMAND_JUMP EXPRESSION_START expression EXPRESSION_END COMMAND_END	#
		jumpToExpression;
