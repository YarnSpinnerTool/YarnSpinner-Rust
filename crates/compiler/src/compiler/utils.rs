...
pub(crate) fn get_document_comments<'input>(
    ...
    let subsequent_comments =
        tokens.get_hidden_tokens_to_right(context.stop().get_token_index(), comments_channel());
    ...
    let preceding_comments =
        tokens.get_hidden_tokens_to_left(context.start().get_token_index(), comments_channel());
...
/// The index of the `COMMENTS` channel, where the lexer places `//` comments.
/// `get_hidden_tokens_to_left`/`_right` take a channel index; antlr4rust emits the
/// `channelNames` table but no per-channel constant, so we resolve it by name.
fn comments_channel() -> i32 {
    yarnspinnerlexer::channelNames
        .iter()
        .position(|&name| name == "COMMENTS")
        .expect("YarnSpinnerLexer must define a COMMENTS channel") as i32
}
