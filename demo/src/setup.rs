...
pub(crate) fn adapt_materials(
    ...
    let glass_path = asset_server
        .get_path(glass_handle.id())
        .unwrap()
        .into_owned();
    let std_label = format!("{}/std", glass_path.label().unwrap());
    let std_handle: Handle<StandardMaterial> = asset_server.load(glass_path.with_label(std_label));
    let mut glass_material = materials.get_mut(&std_handle).unwrap();
